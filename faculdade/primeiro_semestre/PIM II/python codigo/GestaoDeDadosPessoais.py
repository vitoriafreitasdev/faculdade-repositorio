...
class Verificacoes:
    ...
    def verificarRg(self, rg):
        #aqui vamos verificar o comprimento do rg, a quantidade de digitos de um rg é de 8 a 11, aqui contei junto com os "." e "-".
...
#Para fazer os teste digite no terminal: python -m unittest GestaoDeDadosPessoais.py -v        
# Funções para pegar, buscar, salvar, atualizar, editar e excluir os dados
#Pegar
class AlteracoesNosDados:
    ...
    def buscar_dados(self, dados, email, nome):
        ...
        for usuario in dados:
            if usuario.get("email") == email and usuario.get("nome") == nome:
                return usuario
        return None 
    # Salvar
    ...
    # Atualizar
    def atualizar_dados(self, antigoEmail, novoEmail, novaNacionalidade, novoEstadoCivil, nomeDoUsu):  
        #pegamos  os dados
        dados = self.pegar_dados()
        #Agora vamos atualizar os dados
        for i, user in enumerate(dados):
            if user.get("email") == antigoEmail and user.get("nome") == nomeDoUsu:
                dados[i].update({
                "email": novoEmail,
    ...
    def editar_dados(self, email, nome):
        ...
        novoEstadoCivil = input("Novo estado civil (solteiro, casado, divorciado, viúvo ou separado judicialmente): ")
        ...
        self.atualizar_dados(antigoEmail, novoEmail, novaNacionalidade, novoEstadoCivil, nomeDoUsu)
...
while True:
    ...
    if resposta == "1":
        ...
        iniciarCadastro = input("Você autoriza o uso dos seus dados conforme nossa política de privacidade (s/n): ")
